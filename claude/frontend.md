...
- The project editor has Anglais / Français tabs: English holds the shared fields (slug, stack, links, featured) and the default text, French is saved separately through the translation endpoints. Both panes stay mounted so switching tabs never loses unsaved edits. Everything in the admin is usable at 360px (scrolling nav, stacked cards, one-column forms, 44px targets).
...
Only the home page is in the entry bundle. `App.tsx` lazy-loads the whole admin (one `AdminRoutes` chunk mounted on `admin/*`), the project page (it pulls in `react-markdown`) and the two legal pages, behind `Suspense` boundaries (a height-reserving fallback in `RootLayout`, a French "Chargement..." for the admin). Pages use named exports, so each `lazy()` maps them to `default`. Initial home-page JavaScript: about 200 KB gzip, from 246 KB before the split. Keep new admin code reachable only through `AdminRoutes`, otherwise it leaks back into the entry bundle.
